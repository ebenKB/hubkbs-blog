            ...
            while (1) {
              switch (_context2.prev = _context2.next) {
                case 0:
                  ...
                  console.log('this is the user', user); // destructure params from req { req } and pass to the function
                      ...
                      return _regenerator.default.wrap(function _callee$(_context) {
                        while (1) {
                          switch (_context.prev = _context.next) {
                            case 0:
                              console.log('we found a user', aUser);
                              _context.next = 3;
                              return _auth.default.generateToken(aUser);

                            case 3:
                              token = _context.sent;
                              console.log('this is the token from the auth route', token);
                              res.status(200).send({
                                access_token: token,
                                user: user
                              });
                  ...
                  }); // .catch(() => res.status(401).json({ err: 'Sorry we did not find any user' }));
